/*
 * @Author: tetu137 刘永杰
 * @Date: 2023-01-12 19:17:42
 * @LastEditors: [you name]
 * @LastEditTime: 2023-01-16 10:58:27
 * @Description:登录页面
 */
...
class Login extends Component<IProps, IState> {
    ...
    if (document.cookie.length > 0) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (arr2[0] === "userName") {
          // 保存数据并赋值
          this.setState({
            formLogin: { userName: arr2[1], userPwd: formLogin.userPwd },
          });
        } else if (arr2[0] === "userPwd") {
          this.setState({
            formLogin: { userName: formLogin.userName, userPwd: arr2[1] },
          });
        } else {
        }
      }
  ...
  handleLogin = () => {
    ...
    if (!validPass(formLogin.userPwd)) {
      message.error("密码应该为8到20位字母或数字");
      return false;
    }
    // 判断复选框是否被勾选，勾选则调用设置cookie方法
    if (checked) {
      // 传入账号名，密码金和保存天数3个参数
